...
export default function App() {
  ...
  const handleClarify = async (userMessage: string) => {
    ...
    if (!isInitial) {
      dispatch({ type: 'ADD_CLARIFY_MESSAGE', message: { role: 'user', content: userMessage } });
    }
    setClarifyLoading(true);
    try {
      const allMessages = isInitial
        ? []
        : [...state.clarifyMessages, { role: 'user' as const, content: userMessage }];
      ...
      if (ready) dispatch({ type: 'SET_CLARIFY_READY', ready: true });
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Claude returned an unexpected response - try again.';
  ...
  const handleGeneratePost = async () => {
    ...
    try {
      const clarifyingDetails = state.clarifyMessages
        ...
        .map((m) => m.content)
        .join(' | ');
      const { postText } = await api.generatePost(state.postId, state.length, clarifyingDetails || undefined);
